...
def _validate_generation_inputs(
    ...
    _validate_release_metadata(manifest, uv_lock_path, pyproject_path)
...
def generate_prepared_formula(
    manifest: FormulaManifest,
    *,
    uv_lock_path: Path,
    pyproject_path: Path,
) -> str:
    """Render a byte-stable, deliberately non-publishable prepared tap artifact."""
    if manifest.source.status != "prepared":
        raise FormulaParseError("prepared formula generation requires prepared source metadata")
    _validate_release_metadata(manifest, uv_lock_path, pyproject_path)
    install_message = (
        f"AutoBrain {manifest.source.version} is not approved for publication or installation"
    )
    lines = [
        "# NON-PUBLISHABLE PREPARED ARTIFACT. Replace through approved archive generation.",
        "# This file is generated by autobrain-generate-formula --prepared. Do not edit.",
        f"# source status: {manifest.source.status}",
        f"# source url: {manifest.source.url}",
        ...
        f"  version {_ruby_string(manifest.source.version)}",
        f"  sha256 {_ruby_string(manifest.source.sha256)}",
        f"  license {_ruby_string(manifest.license)}",
        "",
        '  disable! date: "2026-08-20", because: "release source is not approved"',
        "",
        "  def install",
        f"    odie {_ruby_string(install_message)}",
        "  end",
        "end",
        "",
    ]
...
def generate_main() -> None:
    ...
    parser.add_argument(
        "--prepared",
        action="store_true",
        help="render an explicitly non-publishable artifact from prepared metadata",
    )
    parser.add_argument(
        "--check-downloads",
        action="store_true",
    ...
    try:
        ...
        if arguments.prepared:
            if arguments.source_archive is not None or arguments.check_downloads:
                raise FormulaParseError(
                    "--prepared cannot be combined with --source-archive or --check-downloads"
                )
            generated = generate_prepared_formula(
                manifest,
