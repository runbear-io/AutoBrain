...
digest uses the release-closure algorithm over `pyproject.toml`, `uv.lock`,
`candidate-pins.json`, and every regular file under `src/autobrain/`, excluding
generated `__pycache__` and `.pyc` files; unrelated evidence, documentation,
test, and release-metadata changes do not alter it. Older or unbound artifacts must remain
